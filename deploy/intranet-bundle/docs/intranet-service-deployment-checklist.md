...
| hostPath 数据目录 | hostPath 数据目录，固定到 `xzyc115-19` | 无 | `/data/observability/<应用名>` | 节点本地目录 | 各组件 Deployment/StatefulSet/Helm values |
| MySQL | RCA investigation 鐑瓨鍌?| `docker-hub.tpo.xzoa.com/auto-inspection/mysql:8.0.26`锛宨nit `docker-hub.tpo.xzoa.com/auto-inspection/docker-hub.tpo.xzoa.com/auto-inspection/busybox:1.36` | NodePort `31326`锛孲ervice `3306` | Kustomize/鎵嬪伐 apply | `source/deploy/db/mysql-31326` |
...
当前内网不再使用 NFS。所有平台持久化数据统一固定到 `xzyc115-19` 节点，工作负载直接使用 `hostPath` volume，不创建 PV/PVC；业务 Pod 通过 `nodeSelector` 固定到 `kubernetes.io/hostname=xzyc115-19` 的内网目标节点。

目录约定：

```text
/data/observability/mysql-31326
/data/observability/prometheus
/data/observability/minio
/data/observability/opensearch
/data/observability/pyroscope
/data/observability/auto-inspection-rca
```
...
|---:|---|---|---|---|---|---|
| 1 | MinIO | Deployment + PVC + Service | investigation 鍐峰瓨鍌ㄣ€佸綊妗ｅ璞″瓨鍌?| `docker-hub.tpo.xzoa.com/auto-inspection/minio:latest` | NodePort `32093` API -> `9000`锛宍32094` Console -> `9001` | `clusters/test/observability/minio` |
| 2 | OpenSearch | StatefulSet + PVC + Service | 鏃ュ織銆佷簨浠躲€乮ncident銆乮nvestigation銆乼race 绱㈠紩瀛樺偍 | `docker-hub.tpo.xzoa.com/auto-inspection/opensearch:2.19.5`锛宨nit `docker-hub.tpo.xzoa.com/auto-inspection/docker-hub.tpo.xzoa.com/auto-inspection/busybox:1.36` | NodePort `32090` -> `9200`锛宮etrics `9600`锛沨eadless `9200/9300/9600` | `clusters/test/observability/opensearch` |
| 3 | OpenSearch Dashboards | Deployment + Service | OpenSearch 鏌ヨ UI | `docker-hub.tpo.xzoa.com/auto-inspection/opensearch-dashboards:2` | NodePort `32091` -> `5601` | `clusters/test/observability/opensearch-dashboards` |
| 4 | Fluent Bit logs | DaemonSet | 閲囬泦鍚勮妭鐐瑰鍣ㄦ棩蹇楀埌 OpenSearch | `docker-hub.tpo.xzoa.com/auto-inspection/fluent-bit:3.2.8` | 瀹瑰櫒 API `2020`锛屾棤 NodePort | `clusters/test/observability/fluent-bit/daemonset-logs.yaml` |
| 5 | Fluent Bit events | Deployment | 閲囬泦 Kubernetes Events 鍒?OpenSearch | `docker-hub.tpo.xzoa.com/auto-inspection/fluent-bit:3.2.8` | 鏃?NodePort | `clusters/test/observability/fluent-bit/deployment-events.yaml` |
| 6 | OTel Collector | Deployment + Service | 鎺ユ敹 Beyla OTLP锛屾毚闇?Prometheus 鎸囨爣鍑哄彛 | `docker-hub.tpo.xzoa.com/auto-inspection/opentelemetry-collector-contrib:0.139.0` | ClusterIP `4317` OTLP gRPC锛宍4318` OTLP HTTP锛宍8889` metrics锛宍13133` health | `clusters/test/observability/otel-collector` |
| 7 | Beyla | DaemonSet | eBPF 鏃犱镜鍏ヤ笟鍔¤皟鐢?RED 鎸囨爣閲囬泦 | `docker-hub.tpo.xzoa.com/auto-inspection/beyla:latest` | 鏃?NodePort锛屽彂閫佸埌 OTel Collector `4318` | `clusters/test/observability/beyla` |
| 8 | Falco | DaemonSet | runtime 瀹夊叏/杩涚▼浜嬩欢璇佹嵁 | `docker-hub.tpo.xzoa.com/auto-inspection/falco:0.43.1` | 瀹瑰櫒 health `8765`锛屾棤 NodePort | `clusters/test/observability/falco` |
| 9 | Pyroscope | Deployment + PVC + Service | profile 瀛樺偍鍜屾煡璇?UI/API | `docker-hub.tpo.xzoa.com/auto-inspection/pyroscope:1.15.1` | NodePort `32095` -> `4040` | `clusters/test/observability/pyroscope/pyroscope-deployment.yaml`锛宍pyroscope-service.yaml` |
| 10 | Alloy Pyroscope eBPF | DaemonSet | eBPF profiling 閲囬泦骞跺啓鍏?Pyroscope | `docker-hub.tpo.xzoa.com/auto-inspection/alloy:latest` | 瀹瑰櫒 HTTP `12345`锛屾棤 NodePort | `clusters/test/observability/pyroscope/alloy-daemonset.yaml` |
| 11 | RCA Backend/MCP | Deployment + PVC + Service | RCA API銆丮CP Server銆丒vidence Pack銆佸彧璇绘帓闅滃叆鍙?| `docker-hub.tpo.xzoa.com/auto-inspection/auto-inspection-rca:20260429-rca-image` | NodePort `32180` Backend -> `18080`锛宍32181` MCP -> `18081` | `clusters/test/observability/auto-inspection-rca` |
| 12 | OpenSearch Snapshot | CronJob | 姣忔棩 OpenSearch snapshot | `docker-hub.tpo.xzoa.com/auto-inspection/python:3.12-slim` | 鏃?| `clusters/test/observability/opensearch-snapshot/cronjob.yaml` |
| 13 | RCA Alert Notify Dry-run | CronJob | 瀹氭椂璋冪敤 `/api/alerts/notify`锛屽綋鍓嶅彧 dry-run 涓嶉€氱煡 | `docker-hub.tpo.xzoa.com/auto-inspection/curl:8.10.1` | 鏃?| `clusters/test/observability/auto-inspection-rca/alert-notify-cronjob.yaml` |
...
docker-hub.tpo.xzoa.com/auto-inspection/minio:latest
docker-hub.tpo.xzoa.com/auto-inspection/opensearch:2.19.5
docker-hub.tpo.xzoa.com/auto-inspection/opensearch-dashboards:2
docker-hub.tpo.xzoa.com/auto-inspection/fluent-bit:3.2.8
docker-hub.tpo.xzoa.com/auto-inspection/opentelemetry-collector-contrib:0.139.0
docker-hub.tpo.xzoa.com/auto-inspection/beyla:latest
docker-hub.tpo.xzoa.com/auto-inspection/falco:0.43.1
docker-hub.tpo.xzoa.com/auto-inspection/pyroscope:1.15.1
docker-hub.tpo.xzoa.com/auto-inspection/alloy:latest
docker-hub.tpo.xzoa.com/auto-inspection/python:3.12-slim
docker-hub.tpo.xzoa.com/auto-inspection/curl:8.10.1
docker-hub.tpo.xzoa.com/auto-inspection/docker-hub.tpo.xzoa.com/auto-inspection/busybox:1.36
